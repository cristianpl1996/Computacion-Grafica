#Modulos
import pygame, sys, random, json, time, threading
...
loadbusters=pygame.sprite.Group()
saluds=pygame.sprite.Group()
energias=pygame.sprite.Group()
...
class X(pygame.sprite.Sprite):
	def __init__(self):
		...
		if not xa:
			x=["images/x_r.png","images/x_l.png","images/x1_r.png","images/x1_l.png","images/xs_r.png","images/xs_l.png"]
		else:	
			x=["images/xa_r.png","images/xa_l.png","images/xa1_r.png","images/xa1_l.png","images/xsa_r.png","images/xsa_l.png"]
		#Mega Man X - X
		self.imageSprite0_r=pygame.image.load(x[0])
		self.imageSprite0_l=pygame.image.load(x[1])
		...
		#Mega Man X - X1.
		self.imageSprite1_r=pygame.image.load(x[2])
		self.imageSprite1_l=pygame.image.load(x[3])
		...
		#Mega Man X - X Saber
		self.imageSprite2_r=pygame.image.load(x[4])
		self.imageSprite2_l=pygame.image.load(x[5])
	...
	def update(self):
		...
		if self.buster>2:
			...
			loadbusters.add(self.loadBuster)
		else:
			loadbusters.remove(self.loadBuster)
...
class Zero(pygame.sprite.Sprite):
	def __init__(self):
		pygame.sprite.Sprite.__init__(self)
		if not zerob:
			zero=["images/zero_r.png","images/zero_l.png","images/zero1_r.png","images/zero1_l.png","images/zeros_r.png","images/zeros_l.png"]
		else:	
			zero=["images/zerob_r.png","images/zerob_l.png","images/zero1_r.png","images/zero1_l.png","images/zerosb_r.png","images/zerosb_l.png"]
		#Mega Man X - Zero
		self.imageSprite0_r=pygame.image.load(zero[0])
		self.imageSprite0_l=pygame.image.load(zero[1])
		imageWidth0=self.imageSprite0_r.get_rect()[2]
		imageHeight0=self.imageSprite0_r.get_rect()[3]
		quantityX0=11
		quantityY0=15
		...
		self.matrix0_l=Cutout(self.imageSprite0_l,cutX0,cutY0,quantityX0,quantityY0)
		#Mega Man X - Zero1.
		self.imageSprite1_r=pygame.image.load(zero[2])
		self.imageSprite1_l=pygame.image.load(zero[3])
		self.matrix1_r=Cutout(self.imageSprite1_r,cutX0,cutY0,quantityX0,quantityY0)
		self.matrix1_l=Cutout(self.imageSprite1_l,cutX0,cutY0,quantityX0,quantityY0)
		#Mega Man X - Zero Saber
		self.imageSprite2_r=pygame.image.load(zero[4])
		self.imageSprite2_l=pygame.image.load(zero[5])
		imageWidth2=self.imageSprite2_r.get_rect()[2]
		imageHeight2=self.imageSprite2_r.get_rect()[3]
		quantityX2=13
		quantityY2=4
...
class Life(pygame.sprite.Sprite):
	def __init__(self,posXY):
		pygame.sprite.Sprite.__init__(self)
		self.imageSprite0=pygame.image.load("images/life.png")
...
class TimeInterludio(threading.Thread):
	def __init__(self,finalMovie):
		threading.Thread.__init__(self)
		self.contador=0
		self.iniciar=False
		self.finalMovie=finalMovie

	def run(self):
		self.iniciar=True
		while True:
			if self.contador>=self.finalMovie:
				break
...
def main():
	global mundo01,mundo02,mundo03,xa,zerob
	#Inicializacion
	pygame.init()
	...
	titlesigma=pygame.image.load("images/titlesigma.png")
	titletime=pygame.image.load("images/titletime.png")
	...
	recolectar=pygame.mixer.Sound("sounds/recolectar.wav")
	...
	p=0
	only=True
	segundos=TimeInterludio(finalMovie)
	t=Time()
	menu.play()
	while not seguir1:
		...
		if tutorial:
			...
			if intro:
				...
				if acabar or (segundos.contador>=finalMovie):
					segundos.reiniciar()
					movie.stop()
					video.stop()
					acabar=False
	...
	salude=Salud()
	sx,sy=0,550
	telonRojo2=pygame.Surface([300,10])
	telonRojo2.fill(ROJO)
	temp=0
	poslife=[]
	t.start()
	Crear=True
	...
	while not seguir3:
		...
		#Logica 
		segundos=str(t.contador)
		contador=fuente.render(segundos,0,(239,184,16))
		salude.actualizar(sigma.salud,sx,sy)
		if Crear:
			if mundo01:
				for e in enemigos:
					...
					if temp==1:
						temp=random.randrange(2)
						if temp==1:
							life=Life((e.rect.x+20,e.rect.y-10))
							life.action=0
						else:
							life=Life((e.rect.x+20,e.rect.y-10))
							life.action=1
		...
		for b in busters:    
			b_col=pygame.sprite.spritecollide(b,enemigos,False)
		...
		for e in energias:
			...
			for en in ls_col:
				if jugador.salud >=100 and jugador.salud<150:
					recolectar.play()
					if e.action==2:
						jugador.salud+=50
					else:
		...
		for e in enemigos:
			ls_col=pygame.sprite.spritecollide(e,jugadores,False)
			for en in ls_col:
				if jugador.action==2 and jugador.i>=10:
					if isinstance(e,Dopler):
						avanzar=True
					if isinstance(e,Sigma):
						termino=True
			...
			if pygame.sprite.collide_circle(e,jugador):
				if e.temp<=0:
					...
					if isinstance(e,Sigma):
						...
						balae.rect.y=e.rect.y+30
						if aleatorio==1:
							balae.image=pygame.image.load("images/b1sigma.png")
						else:	
		...
		if bloquears:
			if jugador.rect.y>HEIGHT or t.contador<=0:
				muerte.play()
				jugador.salud=0
				gameover=True
				jugadores.empty()
		...
		if not gameover:
			if not loading:
				if not pause:
					...
					loadbusters.update()
					loadbusters.draw(pantalla)
					balasE.update()
					balasE.draw(pantalla)
					pantalla.blit(telonNegro,(0,0))
					pantalla.blit(telonRojo,(50,11))
					pantalla.blit(titletime,(290,6))
					pantalla.blit(contador,(330,4))			
					if seleccion==0:
						pantalla.blit(titlex,(0,8))	
					else:
						pantalla.blit(titlezero,(0,8))
					if mundo01:
						pantalla.blit(titlel1,(452,8))
					if mundo02:
						pantalla.blit(titlel2,(452,8))
					if mundo03:
						pantalla.blit(titlefl,(417,6))
						if not termino:
							pantalla.blit(telonRojo2,(sx,sy))
							pantalla.blit(titlesigma,(90,295))	
					saluds.update()
					saluds.draw(pantalla)
					energias.update()
					energias.draw(pantalla)												
					pygame.display.flip()
					reloj.tick(20)
				else:
